...
export function Overview({status}: {status: Status}) {
  ...
  const chartData = (status.recent ?? []).map((item, index) => { const timing = item.timing_ms ?? {}; return { index: index + 1, label: chartLabel(item.ts || item.created_at, index), rewrite: Number(timing.rewrite ?? 0), irodori: Number(timing.irodori_request ?? 0), total: Number(timing.total ?? 0) }; });
  return <section className="tab-view active overview-surface" data-testid="overview-surface">
    ...
    <section className="panel performance-panel dither-panel"><div className="panel-header"><div><div className="eyebrow">OBSERVE / DITHER TRACE</div><h2>Rewrite / Irodori processing</h2></div><span className="panel-hint">直近30件</span></div>
      <div className="chart-card overview-chart"><div className="chart-title"><strong>処理時間の推移</strong><span className="legend-hint">ms · hover for exact values</span></div>
        {chartData.length ? <AreaChart data={chartData} config={{rewrite:{label:'Rewrite',color:'blue'}, irodori:{label:'Irodori',color:'purple'}, total:{label:'Total',color:'green'}}} margins={{top:30,right:20,bottom:28,left:52}} bloom="aura" replayOnDataChange={false} className="overview-chart-canvas"><Grid horizontal vertical={false}/><Area dataKey="total" variant="gradient" strokeVariant="solid"/><Area dataKey="rewrite" variant="dotted" strokeVariant="dashed"/><Area dataKey="irodori" variant="dotted" strokeVariant="solid"/><XAxis dataKey="label" maxTicks={6}/><YAxis tickFormatter={axisDuration}/><Legend isClickable align="right"/><Tooltip labelKey="label" valueFormatter={(value) => axisDuration(value)} /></AreaChart> : <div className="chart-empty">No recent timing data yet.</div>}
      </div>
